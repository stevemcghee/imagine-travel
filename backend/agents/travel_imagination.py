...
def build_travel_agent():
    ...
    # This ensures tools are initialized with the correct environment variables (e.g. from Agent Engine)
    maps_toolset = tools.get_maps_mcp_toolset()
    generate_images = tools.generate_images
    
    # Reload config model if env vars changed? 
    # Usually config.GENAI_MODEL is set at import time. 
    # But we can access os.getenv here if we want to be sure.
    MODEL = os.getenv("GENAI_MODEL", config.GENAI_MODEL)
